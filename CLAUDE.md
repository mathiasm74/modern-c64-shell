...
Caveats: tests drive GETIN -> CHROUT by writing the 10-byte keyboard buffer directly, so the keyboard *matrix* decode and inputs longer than 10 chars still need the GUI (`make run`) or code review. Typing uppercase letters and cursor-left (a shifted key) needs SHIFT support, still a TODO — but the line-editing logic is fully exercised by injecting the codes directly.
