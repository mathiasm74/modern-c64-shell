...
File commands: `rm <name>` (`fs.c`) scratches a file by writing "S0:<name>" to the drive command channel via `iec_command` (`iec.s`, which shares the LISTEN/name/UNLISTEN path with `iec_open`, just a different secondary: $6F vs $F0|sa). Disk-mutating tests work on a throwaway copy of the fixture (`_scratch*.d64`, gitignored) made fresh at module import; `test_rm.py` rm's PROG and confirms ls no longer lists it (README stays).
