...
The board defaults to `fire-24-e`; override for your hardware: `make onerom ONEROM_BOARD=fire-28-a` (`tools/onerom scan --list-boards` lists them). `make onerom-flash` builds and programs a connected device in one step (`tools/onerom program --config-file`); set `ONEROM_SERIAL='5*'` to pick one of several. The build fetches a base firmware release from the One ROM servers (cached after the first run).
